...
### Comparison and Identified Gaps

A feature comparison across the four reviewed platforms and Elera is summarised in Table I (see DOCX). The comparison reveals four gaps that are consistent across all existing tools.
...
The Elera Validation Engine (EVE) implements nine rule categories that check for common electrical errors. Each rule was derived from the most frequent mistakes observed among beginner Arduino users. Rule 1 (Missing Board Detection) checks whether an Arduino Uno is present on the canvas, since without a board no other validation is meaningful. Rule 2 (LED Without Resistor) flags any LED wired to a power source or signal pin without a current-limiting resistor and explains that the LED will draw excessive current and may burn out. Rule 3 (Total Current Overload) sums the current draw of all connected components using the metadata values and checks whether the total exceeds the Arduino Uno's 500 mA USB power limit. Rule 4 (Duplicate Pin Assignment) detects when two or more signal components share the same Arduino I/O pin. Rule 5 (Servo on Serial Pins) warns when a servo motor is connected to pins 0 or 1, which are reserved for serial communication. Rule 6 (Missing Power or Ground) checks whether each component that requires VCC or GND has those connections wired. Rule 7 (Incorrect I2C Pin Wiring) verifies that I2C devices such as the LCD 1602 are connected to pin A4 for SDA and A5 for SCL on the Arduino Uno. Rule 8 (Unconnected Components) flags any component placed on the canvas that has no wires at all. Rule 9 (Floating Signal Pins) identifies partially wired components where signal pins are left unconnected.
...
### Metadata-Driven Component Library
