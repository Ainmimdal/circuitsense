...
1.4.1 Scope
Elera is a client-side web application built around the Arduino Uno. It includes:
...
•  Local saving via browser localStorage.
...
•  Support for other boards such as ESP32 or Raspberry Pi Pico.
•  AI-driven circuit generation (planned for future work).
...
1.  Only the Arduino Uno board is supported. Other boards like ESP32 or Arduino Mega are not included in this version.
2.  Component visuals depend on the @wokwi/elements library. If a component is not available in this library, it cannot be displayed in Elera without building custom rendering.
...
5.  AI integration with Google Gemini is planned for future work but is not part of this deliverable.
...
Requirements were gathered through four methods. First, a literature review of existing circuit design tools (discussed in Chapter 2) helped identify which features are standard in the market and which are missing. Second, unstructured interviews were conducted with lab instructors from the Mechatronic Workshop Lab and System Integration Lab at KICT, IIUM. These interviews focused on the most common wiring mistakes students make during lab sessions and what kinds of feedback would be most helpful. Third, a short survey questionnaire was distributed to peers enrolled in mechatronics courses to understand their experience with existing tools and their preferences for validation features. Fourth, early prototyping of the UI and validation logic provided practical feedback on which rules worked correctly and which needed refinement.
...
Results showed that 60% of respondents rated their satisfaction with error feedback in existing tools at 2 or below on a 5-point Likert scale, with a mean score of 2.31. The most frequently reported method of discovering wiring errors was simulation failure (37%), followed by manual debugging (31%) and instructor intervention (20%). These findings confirm that existing tools do not provide adequate guidance during the design phase.
...
[FIGURE 3.2: System Architecture Diagram. A block diagram showing the ten modules and data flow between them. Component Library feeds metadata to Sidebar, Canvas, Validation Engine, and Auto-Wire Engine. Canvas and Auto-Wire Engine read/write to Store. Store fires structural-change events to Validation Engine. Validation Engine outputs to Validation Bar.]
Use Case Diagram
